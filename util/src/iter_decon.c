...
*		12/18/2009	add -S option to compute S RF.
*		11/26/2012	set o=0 if not keepTime.
*********************************************************************/
...
int main(int argc, char **argv) {
  ...
  for (i=1; !error && i < argc; i++) {
    if (argv[i][0] == '-') {
	 ...
	 sscanf(&argv[i][2],"%f",&tau);
       ...
       case 'F':
	 j = sscanf(&argv[i][2], "%d/%lf/%lf/%ld",&filter,&f1,&f2,&order);
  ...
  if (argc == 1 || error) {
    fprintf(stderr,"usage: %s [-Cmark/t1/t2 -Dtau -Fn/f1[/f2[/order]] (-I|-J) -K[delay] -Nnitr -P -S -Ttaper -V -W src data_filenames ...]\n\
	...
	-F	apply a filter to output (n=1), src (n=2), and data (n=3) (0).\n\
		f1 and f2 are the parameter of the Gaussian low-pass filter and\n\
		pre-signal length in sec if f2 is negative. If f2>0, they\n\
		are the corner freqs. in Hz of the Butterworth band-pass filter.\n\
	...
	-S:	reverse the data to compute S receiver functions.\n\
  ...
  while( (n_files && ++k<argc) || (n_files==0 && fgets(line,512,stdin))) {
    ...
    if (src==NULL) { 	/* input denominator */
	  ...
	  arr=0.;
       ...
       if (filter&&f2>0.) design(order,type,proto,1.,1.,f1,f2,(double)dt,sn,sd,&nsects);
       if (taper>0.) {
       ...
       if (filter>1&&f2>0.) apply((float *)src_spec,(long int)nft,0,sn,sd,nsects);
       ...
       if (revs) revers(res, nn);
       for(j=0;j<nft2;j++) res_spec[j]=Zero;
       memcpy(res_spec, res, nn*sizeof(float));
       if (filter>2&&f2>0.) apply((float *)res_spec,(long int)nft,0,sn,sd,nsects);
       ...
       if (filter) {
          if (f2>0.)
             apply(spikes,(long int)nft,0,sn,sd,nsects);
          else {
